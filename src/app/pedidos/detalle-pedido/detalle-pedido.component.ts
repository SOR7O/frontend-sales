...
export class DetallePedidoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    for (let index = 0; index < this.data["data"].length; index++) {
      to += parseFloat(this.data["data"][index]["total"]["$numberDecimal"]);
      subto += parseFloat(
        this.data["data"][index]["subtotal"]["$numberDecimal"],
      );
      imp += parseFloat(this.data["data"][index]["impuesto"]["$numberDecimal"]);
    }
    this.total = to;
  ...
  generarFactura() {
    this.gpdf.generarPDF(null, "factura", "facturaPedido");
  }
}
